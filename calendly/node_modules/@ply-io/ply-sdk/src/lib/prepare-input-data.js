	...
	for (const key in inputData) {
		if (inputData.hasOwnProperty(key)) {
			...
			if (paramType && paramType.handler) {
				paramType.handler(inputData, key, { previousVars, unflattenPreviousVars, prepareMappedFields });
			} else {
				if (isPlainObject(inputData[key])) {
					const nestedParams = paramTypeUtils.extractParamFromActionParams(actionParams, key).params || [];
