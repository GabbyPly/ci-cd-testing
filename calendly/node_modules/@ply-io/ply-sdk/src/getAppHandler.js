...
const { isNullOrUndefinedOrEmpty } = require('./lib/helpers');
...
const mapResultFromMethod = (mappings, data) => {
	const { results } = data;
	return Object.assign(data, {
		results: results.map((item) => ({
			option: item[mappings.option],
			value: item[mappings.value]
		}))
	});
};

const runMethodOptions = (app, methodName, methodResults, methodOptions) => {
	if (!methodResults) {
		return null;
	}
	...
	if (acMode && methodNameHelper.isActions(methodName)) {
		...
		if (acMappingMethod) {
			if (_.isFunction(acMappingMethod)) {
				newMethodResults = acMappingMethod(methodResults);
			} else if (_.isObject(acMappingMethod)) {
				newMethodResults = mapResultFromMethod(acMappingMethod, methodResults);
			} else {
...
const validateStructure = (app, methodName, methodResults) => {
	if (methodNameHelper.isActions(methodName)) {
		...
		if (isMultiple) {
			if (!methodResults || !methodResults.results || !_.isArray(methodResults.results)) {
...
const executeMethod = async (app, methodName, data, options) => {
	...
	methodResults = runMethodOptions(app, methodName, methodResults, methodOptions);
